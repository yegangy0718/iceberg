...
public class DataStatisticsCoordinatorContext<K> implements AutoCloseable {
  ...
  void sendDataStatisticsToSubtasks(long checkpointId, DataStatistics<K> globalDataStatistics) {
    callInCoordinatorThread(
        () -> {
          ...
          //                    for (final OperatorCoordinator.SubtaskGateway subtaskGateway :
          // subtaskGateways) {
  ...
  void attemptReady(OperatorCoordinator.SubtaskGateway gateway) {
    ...
    System.out.println("ready subtaskGateways " + subtaskGateways);
  }
  ...
  private static class SubtaskGateways {
    ...
    private void registerSubtaskGateway(OperatorCoordinator.SubtaskGateway gateway) {
      ...
      Preconditions.checkState(
          ...
          new Object[] {subtaskIndex, attemptNumber});
    ...
    private OperatorCoordinator.SubtaskGateway getOnlyGatewayAndCheckReady(int subtaskIndex) {
      Preconditions.checkState(
          ...
          new Object[] {subtaskIndex});
      return (OperatorCoordinator.SubtaskGateway)
          Iterables.getOnlyElement(this.gateways[subtaskIndex].values());
    }

    //        private OperatorCoordinator.SubtaskGateway getGatewayAndCheckReady(int subtaskIndex,
    // int attemptNumber) {
    //            OperatorCoordinator.SubtaskGateway gateway =
    // (OperatorCoordinator.SubtaskGateway)this.gateways[subtaskIndex].get(attemptNumber);
    //            if (gateway != null) {
    //                return gateway;
    //            } else {
    //                throw new IllegalStateException(String.format("Subtask %d (#%d) is not ready
    // yet to receive events.", subtaskIndex, attemptNumber));
    //            }
    //        }
