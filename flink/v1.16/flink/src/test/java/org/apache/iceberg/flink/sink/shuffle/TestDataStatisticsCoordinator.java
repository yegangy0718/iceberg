...
public class TestDataStatisticsCoordinator {
  ...
  public void testDataStatisticsEventHandling() throws Exception {
    ...
    MapDataStatistics<String> globalDataStatistics =
        (MapDataStatistics<String>)
            dataStatisticsCoordinator.latestAggregatedDataStatistics().dataStatistics();
    Assert.assertEquals(
  ...
  public void testCleanupExpiredEntryInAggregateDataStatisticsMap() throws Exception {
    ...
    waitForCoordinatorToProcessActions(dataStatisticsCoordinator.context());
    Assert.assertEquals(2, dataStatisticsCoordinator.aggregateDataStatisticsMap().size());
    ...
    // At checkpoint3, coordinator should remove aggregateDataStatistics entry for checkpoint 1
    // since maxAggregateDataDistributionHistoryToKeep = 2
    ...
    checkpoint4Subtask0DataStatistic.add("b");
    checkpoint4Subtask0DataStatistic.add("b");
    ...
    checkpoint4Subtask1DataStatistic.add("a");
    checkpoint4Subtask1DataStatistic.add("a");
    checkpoint4Subtask1DataStatistic.add("b");
    ...
    MapDataStatistics<String> checkpoint5Subtask2DataStatistic = new MapDataStatistics<>();
    checkpoint5Subtask2DataStatistic.add("a");
    DataStatisticsEvent<String> checkpoint5Subtask0DataStatisticEvent =
        new DataStatisticsEvent<>(5, checkpoint5Subtask2DataStatistic);
    // Checkpoint4 data statistics aggregation is done. AggregateDataStatistics entries whose
    // checkpointId
    // are <= 4 should be removed and only keep checkpoint 5
    dataStatisticsCoordinator.handleEventFromOperator(1, 0, checkpoint5Subtask0DataStatisticEvent);
    waitForCoordinatorToProcessActions(dataStatisticsCoordinator.context());
    Assert.assertEquals(1, dataStatisticsCoordinator.aggregateDataStatisticsMap().size());
    Assert.assertTrue(dataStatisticsCoordinator.aggregateDataStatisticsMap().containsKey(5L));
    // Not all subtasks data statistics arrives for checkpoint 5, so completed aggregated data
    // statistics is generated from checkpoint 4
    MapDataStatistics<String> globalDataStatistics =
        (MapDataStatistics<String>)
            dataStatisticsCoordinator.latestAggregatedDataStatistics().dataStatistics();
    Assert.assertEquals(
        checkpoint4Subtask0DataStatistic.dataStatistics().get("a")
            + checkpoint4Subtask1DataStatistic.dataStatistics().get("a"),
        (long) globalDataStatistics.dataStatistics().get("a"));
    Assert.assertEquals(
        checkpoint4Subtask0DataStatistic.dataStatistics().get("b")
            + checkpoint4Subtask1DataStatistic.dataStatistics().get("b"),
        (long) globalDataStatistics.dataStatistics().get("b"));
  }
